...
<template>
  <div class="login-index" :style="backgroundDiv">
    <div class="login-window-index">
      ...
      <!--登录标题-->
      <i style=" top: 20px; font-size: 28px; left: 50px;position: absolute;"class="iconfont">
        <img src="../assets/logo.png" style="width: 60px;position: relative; top: 20px;right: 1px">
        <span style="color: #e75c09;" >爱仁医疗数字化管理平台</span>
      </i>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      findRole: "患者",
      // 公告数据
      announcements: [], // 初始化为空数组
      announcementDialogVisible: false, // 公告弹窗状态
  ...
  mounted() {
    this.fetchAnnouncements(); // 组件挂载时获取公告
  },
  methods: {

    fetchAnnouncements() {
      request
        .get("/announcement/findAll") // 使用 API 接口获取最新公告
        .then((res) => {
          this.announcements = res.data.data; // 更新公告数据
          if (!res.data || res.data.length === 0) {
            console.error("公告数据为空或未找到公告");
          }
        })
        .catch((error) => {
